...
### Check WhatsApp Capability Before Sending

```javascript
// 1. Initiate capability check
const capabilityRequest =
  await sinchClient.conversation.capability.queryCapability({
    projectId: "PROJECT123",
    queryCapabilityRequestBody: {
      app_id: "01H1234567...",
      recipient: {
        contact_id: "01H3333333...",
      },
      request_id: "custom_request_abc123",
      channel: "WHATSAPP",
    },
  });

// 2. Webhook fires with result (this example shows the webhook payload)
// Handle in your webhook endpoint:
app.post("/webhook", (req, res) => {
  const notification = req.body.capability_notification;

  if (notification.capability_status === "CAPABILITY_FULL") {
    // WhatsApp available - send rich message
    sendWhatsAppMessage(notification.contact_id);
  } else {
    // WhatsApp not available - fallback to SMS
    sendSMS(notification.contact_id);
  }
