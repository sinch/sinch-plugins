...
(function main() {
  ...
  var projectId = process.env.SINCH_PROJECT_ID;
  var keyId = process.env.SINCH_KEY_ID;
  var keySecret = process.env.SINCH_KEY_SECRET;
  var appId = process.env.SINCH_APP_ID;
  var region = process.env.SINCH_REGION || "us";

  if (!projectId || !keyId || !keySecret || !appId) {
    console.error(
      "Error: SINCH_PROJECT_ID, SINCH_KEY_ID, SINCH_KEY_SECRET, and SINCH_APP_ID environment variables are required",
    );
    process.exit(1);
  }
