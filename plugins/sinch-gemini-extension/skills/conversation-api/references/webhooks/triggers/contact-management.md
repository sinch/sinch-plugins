...
```
1. Contact created in Conversation API → CONTACT_CREATE fires
   - Create matching contact in CRM
   - Store Conversation API contact_id in CRM

2. Customer adds email via self-service → CONTACT_UPDATE fires
   - Update email in CRM record
   - Enable email communication channel

3. Customer requests data deletion → CONTACT_DELETE fires
   - Delete from CRM
...
```
1. Same phone number assigned to 2 contacts → CONTACT_IDENTITIES_DUPLICATION fires
   - Alert data quality team
   - Review both contacts

2. Team merges contacts → CONTACT_MERGE fires
   - Merge CRM records
   - Update all references to use preserved contact ID
```

### Multi-Channel Contact Growth

```
1. Customer sends WhatsApp message → CONTACT_CREATE fires (auto-created)
   - Create contact with WhatsApp identity only

2. Customer provides phone number → CONTACT_UPDATE fires
   - Add SMS channel identity
   - Enable SMS fallback communication
```
