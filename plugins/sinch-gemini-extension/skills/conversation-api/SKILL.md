...
When the user chooses **SDK**, refer to the [sinch-sdks](../sinch-sdks/SKILL.md) skill for installation, client initialization, and language-specific references.
...
See [sinch-authentication](../sinch-authentication/SKILL.md) for full setup. Uses project-scoped credentials (Project ID + Key ID + Key Secret). OAuth2 bearer token recommended for production; Basic auth for quick testing only (heavily rate-limited).
